...
export class Helpers {
  ...
  public async getAuthUserByUsernameForToken(username: string): Promise<any | null> {
    if (!username) {
      throw new Error('Username is required');
    }
    // const user: any | null = await AuthModel.findOne({ firstName: Helpers.firstLetterUppercase(username) }).exec();
    // return user;
  }
